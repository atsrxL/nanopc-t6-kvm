...
最新结果见 [实机记录](../results/20260923-live/STATUS.md)。上表是初始交接状态，不能覆盖本次证据：完整固定源码编译、部署、配置导入、TLS/认证/接管协议测试、无信号提示及停止清理已通过。HDMI 无信号、USB 未连接，视频硬编解码、TigerVNC、帧率、实际键鼠、热插拔与长稳仍未验收。

## 最终 1080p 可用验证

部署 dev20260923g：真实 MPP、1978 帧 VNC 码流独立解码、TigerVNC 1.15.0 显示、PC evdev 键盘及鼠标事件、认证接管通过。实测约 58–59 fps；不宣称严格无丢帧 60fps。最终同一 epoch 超过 14000 帧、零 worker 重启。1440p、物理热插拔及小时级长稳仍未验收。详见 STATUS.md 和 USAGE.md。
